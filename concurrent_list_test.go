...
func checkListPointers(t *testing.T, l *List, es []*Element) {
	...
	if len(es) == 0 {
		if head.next != tail {
			t.Errorf("l.head.next = %p; should both be %p", head.prev, tail)
		}
		if tail.prev != head {
			t.Errorf("l.tail.prev = %p; should both be %p", tail.prev, head)
		}
		return
	}
	// len(es) > 0
...
func TestList(t *testing.T) {
	...
	if f := l.Front(); f != nil {
		t.Errorf("l.Front() is %p, want nil", f)
	}
	if b := l.Back(); b != nil {
		t.Errorf("l.Back() is %p, want nil", b)
	}
	...
	if rSum != 4 {
		t.Errorf("sum over l = %d, want 4", rSum)
	}
